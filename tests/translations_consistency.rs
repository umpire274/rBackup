...
fn translations_have_same_keys() {
    ...
    let v: serde_json::Value =
        serde_json::from_str(data).expect("translations.json must be valid JSON");
    let obj = v
        .as_object()
        .expect("translations.json must be a JSON object at top-level");
    ...
    for (lang, val) in obj.iter() {
        let map = val
            .as_object()
            .unwrap_or_else(|| panic!("language '{}' must map to an object", lang));
        ...
        if let Some(ref e) = expected {
            ...
            assert!(
                ...
                "Language '{}' has inconsistent keys. Missing: {:?}, Extra: {:?}",
                lang,
                missing,
                extra
            );
